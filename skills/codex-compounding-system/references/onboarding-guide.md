...
Use this guide when Codex 复利系统 is installed for a new user, new computer, or new Codex setup.
...
- **Review scope:** Codex work records and local process artifacts.
- **Cloud content:** record links only until the user explicitly authorizes reading.
- **Process artifacts:** save under the current Codex workbench, recommended folder `Codex复利系统/过程产物/`.
...
```text
数字资产库是长期保存成果的地方；复盘过程产物会留在 Codex 工作台，不会默认混进去。
```
...
If the user already has one, record the path. If not, recommend a simple location under the user's Codex workbench or documents directory.
...
```text
第 5/5 步：我们先跑一天小样。你想复盘哪一天？推荐先用昨天；如果昨天没怎么使用 Codex，我也可以帮你找最近工作量比较高的一天。
```

If the chosen day has little or no evidence, do not force a low-value review. Tell the user:

```text
这一天可复盘材料很少。为了让你看清这个系统的效果，建议换最近 Codex 工作量更高的一天先跑小样。
```
...
- 工作台过程产物目录：
- 时区：
- 证据来源：默认读取 Codex 工作记录和本地过程产物；云端内容授权后才读取
- 审批方式：本地 HTML 审批台
...
我将使用这些设置：
- 数字资产库：
- 过程产物目录：当前 Codex 工作台下的 Codex复利系统/过程产物/
- 时区：
- 证据来源：Codex 工作记录和本地过程产物；云端内容只记录链接，授权后才读取
