...
function collectApprovalResult() {
  ...
  return {
    ...
    decisions: cards.map((card) => {
      ...
      const apcManual = card.querySelector(`input[name="apc-manual-${CSS.escape(id)}"]:checked`)?.value || card.dataset.defaultApcManualAction || "";
      ...
      return {
        ...
        apcMaintenanceManualAction: apcManual,
        note,
        changed: card.dataset.defaultPrimaryAction !== primary || card.dataset.defaultAssetAction !== asset || card.dataset.defaultApcManualAction !== apcManual || note.trim().length > 0
      };
    })
  };
}
